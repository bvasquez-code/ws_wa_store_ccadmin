...
public class TransferCreateService extends SessionService {
    ...
    private TransferRegisterBundleDto save(TransferRegisterBundleDto request, boolean isCreate) throws Exception {
        ...
        if (TransferConstants.TYPE_OPERATION_SEND.equals(typeOperation)) {
            ...
            if (existing != null) {
                if (TransferConstants.STATUS_DISPATCHED.equals(existing.TransferStatus)
                        || TransferConstants.STATUS_FINALIZED.equals(existing.TransferStatus)) {
    ...
    public ResponseWsDto dispatchTransfer(TransferDispatchDto request) throws Exception {
        ...
        if (TransferConstants.STATUS_DISPATCHED.equals(head.TransferStatus)
                || TransferConstants.STATUS_FINALIZED.equals(head.TransferStatus)) {
        ...
        List<TransferDetEntity> detList = this.transferDetRepository.findByTransferCodAndTypeOperation(
                request.transferCod, TransferConstants.TYPE_OPERATION_SEND
        );
        if (detList.isEmpty()) {
        ...
        head.TransferStatus = TransferConstants.STATUS_DISPATCHED;
    ...
    public ResponseWsDto receiveTransfer(TransferReceiveDto request) throws Exception {
        ...
        if (head == null) {
            throw new TransferException("No existe transferencia TS para recepción");
        }

        if (TransferConstants.STATUS_FINALIZED.equals(head.TransferStatus)) {
            return new ResponseWsDto("La transferencia ya fue recibida");
        }
        if (!TransferConstants.STATUS_DISPATCHED.equals(head.TransferStatus)) {
            throw new TransferException("No se puede recibir transferencia que no está despachada");
        }
        ...
        head.TransferStatus = TransferConstants.STATUS_FINALIZED;
        if (StringUtil.isNotEmpty(request.observation)) {
            head.Observation = request.observation;
        }
        head.addSession(getUserSession(request.user), false);
    ...
    private ResponseWsDto changeStatus(TransferReceiveDto request, String status, String message) throws Exception {
        ...
        TransferHeadEntity headRequest = this.transferHeadRepository.findByTransferCodAndTypeOperation(
                request.transferCod, TransferConstants.TYPE_OPERATION_REQUEST
        );
        if (headRequest == null) {
        ...
        if (TransferConstants.STATUS_DISPATCHED.equals(headRequest.TransferStatus)
                || TransferConstants.STATUS_FINALIZED.equals(headRequest.TransferStatus)) {
            throw new TransferException("No se puede modificar transferencia ya despachada");
        }
        if (headSend != null && (TransferConstants.STATUS_DISPATCHED.equals(headSend.TransferStatus)
                || TransferConstants.STATUS_FINALIZED.equals(headSend.TransferStatus))) {
            throw new TransferException("No se puede modificar transferencia ya despachada");
        }
